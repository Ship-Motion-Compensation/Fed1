...
def get_rmse(param, reframed):
    ...
    # training
    history = model.fit(train_X, train_y, epochs=100, batch_size=256, validation_data=(valid_X, valid_y), verbose=0,
                        shuffle=True)
...
reframed.drop(reframed.columns[
                  [index_col + 1, index_col + 2, index_col + 3, index_col + 4, index_col + 5, index_col + 6,
                   index_col + 7]], axis=1, inplace=True)
