...
#读取数据
data_csv_0 = pd.read_csv(f, usecols=[3])    #读取第一列数据
t0 = pd.read_csv(f, usecols=[4])
T0 = pd.read_csv(f, usecols=[5])
...
data_csv_1 = pd.read_csv(f, usecols=[3])    #读取第一列数据
T1 = pd.read_csv(f, usecols=[5])
t1 = pd.read_csv(f, usecols=[4])
...
#进行测试
data0 = np.zeros(len(Data1)*2)
data0.dtype = 'float64'
data0 = data0.reshape(len(Data1),2)
...
# a = len(data0)//10
for i in range(0, (len(data0)-I-O-1)//O):
    data01 = NormalizeMultUseData(data0[i*O: (i*O+I)], normalize)
...
plt.rcParams['axes.unicode_minus'] = False
plt.title("预测")
plt.plot(y1,y[:,1],'r',label='prediction')
