...
def train_test(f):
    ...
    # plt.savefig(F + "\本地训练" + name + ".png", dpi=600)
    plt.show()
    return D_d01, D_T01, D_t01, D01, model0, rmse
...
if __name__=="__main__":
    ...
    while True:
        i +=1
        D_d01, D_T01, D_t01, D01, model0, rmse = train_test(f1)
        if m >= len(D01):
            m = len(D01)
        if float(rmse) <= rmse1:
            rmse1 = float(rmse)
    ...
    while True:
        i0 += 1
        model1, rmse_1 = train_text2(D_d01, D_T01, D_t01, D_1)
        if float(rmse_1) <= rmse2:
            rmse2 = float(rmse_1)
            model10 = model1
        if i0 == 2:
            break

    D_d02, D_T02, D_t02, D02 = text(f3, model)
    text2(D_d02, D_T02, D_t02, D02, model10)
