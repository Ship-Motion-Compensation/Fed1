...
def train_test(f, name):
    ...
    model0 = trainModel(train_X0, train_Y0, name)
    ...
    # 训练
    data1, normalize1, min1 = NormalizeMult(data1)
    train_X1, train_Y1 = create_dataset(data1, I, O)
    model1 = trainModel(train_X1, train_Y1, name)
    # 创建测试数组
    y11 = [[0, 0, 0]]
    y11_2 = []
    for i in range((len(Data1) - I)):
        ...
        a = t1[i:i + I][-1]
        # 测试
        data11, normalize11, min11 = NormalizeMult(data11)
        ...
        y_hat11 = FNormalizeMult(y_hat11, normalize11, min11)  # 反归一化
    ...
    plt.subplot(3, 1, 1)
    plt.title(name + "训练集预测")
    plt.plot(y11[:, 0], y11[:, 1]+y11[:,2], color='k', linewidth=1.0)
    ...
    for i in range((len(Data12) - I)):
        ...
        a = t12[i:i + I][-1]
        # 测试
        data12, normalize12, min12 = NormalizeMult(data12)
        ...
        y_hat12 = FNormalizeMult(y_hat12, normalize12, min12)  # 反归一化
    ...
    plt.subplot(3, 1, 1)
    plt.title(name + "测试集预测")
    plt.plot(y12[:, 0], y12[:, 1]+y12[:,2], color='k', linewidth=1.0)
