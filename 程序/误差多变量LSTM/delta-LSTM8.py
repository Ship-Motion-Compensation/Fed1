...
F = r"C:\Users\LDK\Desktop\da-tu\LSTM\2"  # 图片保存位置
f_1 = r"C:\Users\LDK\Desktop\da-tu\LSTM\2\rmse.txt"  # print输出保存位置
N1 = 20  # 两变量LSTM训练次数
N2 = 1  # 三变量LSTM训练次数
M = 5  # 训练步长
I = 4  # 输入
...
def trainModel(train_X, train_Y, N):
    ...
    model.add(LSTM(
        ...
        return_sequences=False))  # return_sequences神将网络的层数，不是最后一层=true，最后一层=false
...
def train_test(f):
    ...
    data_csv_0111 = pd.read_csv(f, usecols=[0])

    Data0 = data_csv_0.values.flatten()
    t = data_csv_0111.values.flatten()
...
def train_text2(D_d01,D01):
    ...
    D11_1 = data1[:, 0][I:] - (y11[1:][:,0]+y11[1:][:,1])
    ...
    plt.plot(y11[1:][:,0]+y11[1:][:,1], color='gray', linewidth=1.0)
...
def text2(D_d02, D02, model1):
    ...
    D12_1 = data120[:, 0][I:] - (y12[1:][:, 0] + y12[1:][:, 1])
    ...
    plt.plot((y12[1:][:, 0] + y12[1:][:, 1]), color='gray', linewidth=1.0)
