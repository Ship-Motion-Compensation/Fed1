...
def train_test(f):
    ...
    # 创建训练数组
    data00 = np.zeros(len(Data0) * 1)
    data00.dtype = 'float64'
    data00 = data00.reshape(len(Data0), 1)
    data00[:, 0] = Data0  # 位移
    # 画图
    plt.subplot(3, 1, 1)
    plt.title("训练数据")
    plt.plot(data00[:, 0],color='k',linewidth=1.0)
    plt.xlabel("时间/s")
    plt.ylabel("波高/m")
    plt.subplot(3, 1, 3)
    plt.title("训练数据时间差")
    plt.plot(data00[:, 0][1:],color='k',linewidth=1.0)
    plt.xlabel("个数")
    plt.ylabel("时间/s")
    ...
    # 训练
    data0, normalize0 = NormalizeMult(data00)
...
def train_text2(D_d01,D01):
    ...
    # 创建训练数组
    data100 = np.zeros(len(Data1) * 2)
    data100.dtype = 'float64'
    data100 = data100.reshape(len(Data1), 2)
    # data1[:, 0] = T1  # 时间
    data100[:, 0] = Data1  # 位移
    data100[:, 1] = delta_t1[:,0]    # 误差
    # 画图
    plt.subplot(3, 1, 1)
    plt.title("本地训练数据")
    plt.plot(data100[:, 0],color='k',linewidth=1.0)
    plt.xlabel("时间/s")
    plt.ylabel("波高/m")
    plt.subplot(3, 1, 3)
    plt.title("训练数据误差")
    plt.plot(data100[:, 1],color='k',linewidth=1.0)
    plt.xlabel("个数")
    plt.ylabel("时间/s")
    ...
    # 训练
    data1, normalize1 = NormalizeMult(data100)
    ...
    D11 = data100[:, 0][I:] - y11[1:][:,0]
    D11_1 = data100[:, 0][I:] - (y11[1:][:,0]+y11[1:][:,1])
