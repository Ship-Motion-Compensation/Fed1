...
def model_attention_applied_before_lstm():
    ...
    output = Dense(K_IMF, activation='sigmoid')(attention_mul)
...
def train_test(f):
    ...
    Data0, u_hat, omega = vmd(Data00)

    # plt.figure(figsize=(K_IMF, K_IMF), dpi=200)
    for ii in range(K_IMF):
    ...
    # 创建训练数组
    data0 = np.zeros(len(Data0[0]) * K_IMF)
    data0.dtype = 'float64'
    data0 = data0.reshape(len(Data0[0]), K_IMF)
...
def text(f3, model0):
    ...
    Data002 = data_csv_02.values.flatten()[2000:2500]
    Data02, u_hat, omega = vmd(Data002)

    data020 = np.zeros(len(Data02[0]) * K_IMF)
    data020.dtype = 'float64'
    data020 = data020.reshape(len(Data02[0]), K_IMF)
    for i0 in range(K_IMF):
