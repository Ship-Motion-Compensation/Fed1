...
if __name__=="__main__":
    ...
    Y_dan1 = []
    Y_dan2 = []
    for i_dan1 in range(len(Data0[0][:1000])-I):
        Y_dan1.append(0)
    for i_dan2 in range(len(Data0[0][-500:])-I):
        Y_dan2.append(0)

    for ia in range(len(Data0)):
        D_d01, D01, model0, y1 = train_test(Data0[ia][:1000])
        Y_dan1 = np.array(Y_dan1) + np.array(y1)
        D_d02, D02, y2 = text(Data0[ia][-500:], model0)
        Y_dan2 = np.array(Y_dan2) + np.array(y2)

    e_1 = np.array(Data00[:1000][I:]) - Y_dan1
