...
f1 = '仿真5-60-6.csv'  # A数据
f3 = '仿真5-60-6.csv'     # 测试集数据
F = r"C:\Users\刘敦康\Desktop\datu\new"  # 图片保存位置
f_1 = r"C:\Users\刘敦康\Desktop\datu\new\rmse.txt"  # print输出保存位置
N1 = 5  # 两变量LSTM训练次数
N2 = 20  # 三变量LSTM训练次数
M = 5  # 训练步长
I = 4  # 输入
...
def model_attention_applied_before_lstm0():
    K.clear_session() #清除之前的模型，省得压满内存
    inputs = Input(shape=(I, INPUT_DIM,))
    attention_mul = attention_3d_block0(inputs)
    lstm_units = 32
...
def trainModel0(train_X, train_Y, N):
    '''
    trainX，trainY: 训练LSTM模型所需要的数据
    '''
    model = Sequential()
...
def train_test(DATA,T):
    # 读取数据(训练)
    # data_csv_0 = pd.read_csv(f, usecols=[4])  # 读取第一列数据  位移

    # Data0 = data_csv_0.values.flatten()[:1000]
    Data0 = DATA
    T0 = T
    ...
    plt.show()
    return Data0[:-I], D01, model0, y01_20,T0

def text(Data02,T0, model0):
    # 读取数据(预测)
    # data_csv_02 = pd.read_csv(f3, usecols=[4]).dropna()  # 读取第一列数据

    # Data02 = data_csv_02.values.flatten()[-500:]
    ...
    T020 = np.zeros(len(T0) * 1)
    T020.dtype = 'float64'
    T020 = T020.reshape(len(T0), 1)
    T020[:, 0] = T0  # 位移
    ...
    plt.subplot(3, 1, 1)
    # plt.title("测试集预测")
    plt.plot(T020[:-I],y02_2, color='r', linewidth=1.0, label='预测值')
    plt.plot(T020[:-I],data020[:-I], color='b', linewidth=1.0, label='实际值')
    plt.legend(loc=1)
    plt.xlabel("时间/s")
    plt.ylabel("倾角/°")
    plt.subplot(3, 1, 3)
    ...
    plt.xlabel("时间/s")
    plt.ylabel("倾角/°")
    plt.savefig(F + "\单变量测试数据"+str(ia)+"预测图.png", dpi=600)
    plt.show()

    return Data02[I:], D02, y02_20
...
def train_text2(D_d01,D01):
    ...
    model1 = trainModel0(train_X1, train_Y10, N2)
...
if __name__=="__main__":
    ...
    writer0 = pd.ExcelWriter('A0.xlsx')
    writer1 = pd.ExcelWriter('A1.xlsx')
    writer2 = pd.ExcelWriter('A2.xlsx')
    writer3 = pd.ExcelWriter('A3.xlsx')

    data_csv_0 = pd.read_csv(f1, usecols=[4])  # 读取第一列数据  位移
    t_0 = pd.read_csv(f1, usecols=[0])
    Data00 = data_csv_0.values.flatten()
    T0 = t_0.values.flatten()

    Data0, u_hat, omega, res = vmd(Data00)
    Data0 = np.vstack((Data0, res))
    Y_dan1 = []
    Y_dan2 = []
    for i_dan1 in range(len(Data0[0][:2000])-I):
        Y_dan1.append(0)
    for i_dan2 in range(len(Data0[0][-500:])-I):
        Y_dan2.append(0)

    for ia in range(len(Data0)):
        D_d01, D01, model0, y1,T = train_test(Data0[ia][:2000],T0[:2000])
        Y_dan1 = np.array(Y_dan1) + np.array(y1)
        D_d02, D02, y2 = text(Data0[ia][-500:],T0[-500:], model0)
        Y_dan2 = np.array(Y_dan2) + np.array(y2)

    e_1 = np.array(Data00[:2000][:-I]) - Y_dan1
    e_2 = np.array(Data00[-500:][:-I]) - Y_dan2
    rmse_1 = '%.5f' % (math.sqrt(sum([x ** 2 for x in e_1]) / len(e_1)))
    rmse_2 = '%.5f' % (math.sqrt(sum([x ** 2 for x in e_2]) / len(e_2)))
    print(rmse_1, rmse_2)

    DDD0 = pd.DataFrame(Data00[-500:][:-I])
    DDD0.to_excel(writer0, 'page_1', float_format='%.5f')
    writer0.save()
    DDD1 = pd.DataFrame(Y_dan2)
    DDD1.to_excel(writer1, 'page_1', float_format='%.5f')
    writer1.save()
    DDD2 = pd.DataFrame(e_2)
    DDD2.to_excel(writer2, 'page_1', float_format='%.5f')
    writer2.save()
    DDD3 = pd.DataFrame(T0[-500:][:-I])
    DDD3.to_excel(writer3, 'page_1', float_format='%.5f')
    writer3.save()

    plt.subplot(3, 1, 1)
    plt.title("训练集预测")
    plt.plot(T0[:2000][:-I],Data00[:2000][:-I], color='k', linewidth=1.0, label='实际值')
    plt.plot(T0[:2000][:-I],Y_dan1, color='k', linewidth=1.0, label='预测值')
    plt.legend(loc=1)
    plt.xlabel("时间/s")
    plt.ylabel("倾角/°")
    ...
    plt.subplot(3, 1, 3)
    plt.title("训练集预测")
    plt.plot(T0[-500:][:-I], Data00[-500:][:-I], color='k', linewidth=1.0, label='实际值')
    plt.plot(T0[-500:][:-I], Y_dan2, color='gray', linewidth=1.0, label='预测值')
    plt.legend(loc=1)
    plt.xlabel("时间/s")
    plt.ylabel("倾角/°")
    plt.savefig(F + "\最终预测图.png", dpi=600)
    plt.show()
