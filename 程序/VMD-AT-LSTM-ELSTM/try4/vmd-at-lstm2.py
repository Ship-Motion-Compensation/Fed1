...
def train_test(f):
    ...
    data_csv_0001 = pd.read_csv(f, usecols=[0])

    Data00 = data_csv_0.values.flatten()[:2000]
    t = data_csv_0001.values.flatten()[:2000]
    ...
    for ii in range(K_IMF + 1):
        if ii <= K_IMF - 1:
            ...
            plt.plot(t, Data0[ii, :], linewidth=0.8, c='k')
            ...
            plt.xlabel("时间/s")
            plt.ylabel('res')
    plt.savefig(F + "\训练数据分解图.png", dpi=600)
    plt.show()
    ...
    for ii0 in range(K_IMF + 1):
        ...
        if ii0 <= K_IMF - 1:
            ...
            plt.plot(Data0[ii0, :], linewidth=0.8, c='gray', label='预测值')
            plt.plot(c, linewidth=0.8, c='k', label='实际值')
            # plt.legend(loc=1)
            plt.ylabel('IMF{}'.format(ii0 + 1))
        else:
            plt.subplot(K_IMF + 1, 1, K_IMF + 1)
            plt.plot(res, linewidth=0.8, c='gray', label='预测值')
            plt.plot(c, linewidth=0.8, c='k', label='实际值')
            plt.legend(loc=1)
            plt.xlabel("时间/s")
            plt.ylabel('res')
    plt.savefig(F + "\分解训练数据预测图.png", dpi=600)
    plt.show()

    plt.subplot(3, 1, 1)
    plt.title("训练集预测")
    plt.plot(t[:-I], y01_2, color='gray', linewidth=1.0, label='预测值')
    plt.plot(t, Data00, color='k', linewidth=1.0, label='实际值')
    plt.legend(loc=1)
    plt.xlabel("时间/s")
    plt.ylabel("倾角/°")
    plt.subplot(3, 1, 3)
...
def text(f3, model0):
    ...
    Data002 = data_csv_02.values.flatten()[-500:]
    t = data_csv_0211.values.flatten()[-500:]
    ...
    for ii0 in range(K_IMF + 1):
        ...
        if ii0 <= K_IMF - 1:
            plt.subplot(K_IMF + 1, 1, ii0 + 1)
            plt.plot(t, Data02[ii0, :], linewidth=0.8, c='r')
            plt.plot(t[:-I], c, linewidth=0.8, c='b')
            plt.ylabel('IMF{}'.format(ii0 + 1))
        else:
            plt.subplot(K_IMF + 1, 1, K_IMF + 1)
            plt.plot(t, res, linewidth=0.8, c='r', label='预测值')
            plt.plot(t[:-I], c, linewidth=0.8, c='b', label='实际值')
            plt.legend(loc=1)
            plt.ylabel('res')
            plt.xlabel("时间/s")
    plt.savefig(F + "\分解预测数据预测图.png", dpi=600)
    plt.show()

    DDD0 = pd.DataFrame(Data002)
    ...
    plt.plot(t, Data002, color='k', linewidth=1.0, label='实际值')
    plt.legend(loc=1)
    plt.xlabel("时间/s")
    plt.ylabel("倾角/°")
    plt.subplot(3, 1, 3)
