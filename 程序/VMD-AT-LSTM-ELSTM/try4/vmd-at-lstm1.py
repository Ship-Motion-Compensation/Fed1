...
f1 = '仿真6-60-2-0.csv'  # A数据
f3 = '仿真6-60-2-0.csv'     # 测试集数据
F = r"C:\Users\LDK\Desktop\da-tu\VMD-AT-LSTM\5"  # 图片保存位置
f_1 = r"C:\Users\LDK\Desktop\da-tu\VMD-AT-LSTM\5\rmse.txt"  # print输出保存位置
N1 = 15  # 两变量LSTM训练次数
N2 = 20  # 三变量LSTM训练次数
M = 5  # 训练步长
I = 4  # 输入
...
def train_test(f):
    ...
    for ii0 in range(K_IMF+1):
        ...
        if ii0 <= K_IMF-1:
            ...
            plt.plot(Data0[ii0, :], linewidth=0.8, c='gray')
            plt.plot(c, linewidth=0.8, c='k')
            plt.ylabel('IMF{}'.format(ii0 + 1))
        else:
            plt.subplot(K_IMF + 1, 1, K_IMF + 1)
            plt.plot(res, linewidth=0.8, c='gray')
            plt.plot(c, linewidth=0.8, c='k')
            plt.ylabel('res')
...
def text(f3, model0):
    ...
    Data002 = data_csv_02.values.flatten()[400:1200]
    t = data_csv_0211.values.flatten()[400:1200]
