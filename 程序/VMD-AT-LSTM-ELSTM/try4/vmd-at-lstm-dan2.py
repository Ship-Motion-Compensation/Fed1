...
F = r"C:\Users\LDK\Desktop\E-LSTM\000"  # 图片保存位置
f_1 = r"C:\Users\LDK\Desktop\E-LSTM\000\rmse.txt"  # print输出保存位置
N1 = 1  # 两变量LSTM训练次数
N2 = 20  # 三变量LSTM训练次数
M = 5  # 训练步长
I = 5  # 输入
...
def train_test(DATA):
    ...
    plt.show()
    return Data0[I:], D01, model0, y01_20

def text(Data02, model0):
    # 读取数据(预测)
...
if __name__=="__main__":
    ...
    for i_dan1 in range(len(Data0[0][:2000])-I):
        Y_dan1.append(0)
    for i_dan2 in range(len(Data0[0][2000:2500])-I):
        Y_dan2.append(0)

    for ia in range(len(Data0)):
        # D_d01, D01, model0, y1 = train_test(Data0[ia][:2000])
        exec("D_d01, D01, model%s, y1 = train_test(Data0[ia][:2000])" % (ia))
        Y_dan1 = np.array(Y_dan1) + np.array(y1)
        exec("D_d02, D02, y2 = text(Data0[ia][2000:2500], model%s)" % (ia))
        Y_dan2 = np.array(Y_dan2) + np.array(y2)

    e_1 = np.array(Data00[:2000][:-I]) - Y_dan1
    e_2 = np.array(Data00[2000:2500][:-I]) - Y_dan2
    ...
    plt.subplot(3, 1, 1)
    plt.title("训练集预测位移+误差")
    plt.plot(Data00[:2000][:-I], color='k', linewidth=1.0)
    plt.plot(Y_dan1, color='k', linewidth=1.0)
    plt.xlabel("时间/s")
    plt.ylabel("波高/m")

    plt.subplot(3, 1, 3)
    plt.title("训练集预测位移+误差")
    plt.plot(Data00[2000:2500][:-I], color='k', linewidth=1.0)
    plt.plot(Y_dan2, color='k', linewidth=1.0)
    plt.xlabel("时间/s")
    plt.ylabel("波高/m")
    plt.show()
