...
f1 = r'仿真5-60-6.csv'  # A数据
f3 = r'仿真5-60-6.csv'     # 测试集数据
...
def train_test(f):
    ...
    t = data_csv_011.values.flatten()[:2000]
...
def train_text2(D01,t):
    ...
    model1 = trainModel0(train_X1, train_Y1, N2)
...
def text2(D02, model1,t):
    ...
    plt.subplot(3, 1, 1)
    plt.title("测试集预测")
    plt.plot(t[:-2*I], y12[1:][:, 0], color='gray', linewidth=1.0, label='预测值')
    plt.plot(t[:-2*I], Data12[:-I], color='k', linewidth=1.0, label='实际值')
    plt.legend(loc=1)
    plt.xlabel("时间/s")
    plt.ylabel("倾角/°")
    plt.subplot(3, 1, 3)
...
if __name__=="__main__":
    f0 = open(f_1, "w")
    writer0 = pd.ExcelWriter('A00.xlsx')
    writer1 = pd.ExcelWriter('A11.xlsx')
    writer2 = pd.ExcelWriter('A22.xlsx')
    writer3 = pd.ExcelWriter('A33.xlsx')
    ...
    plt.subplot(3, 1, 1)
    plt.title("训练集预测")
    plt.plot(t2[:-2 * I], result1, color='gray', linewidth=1.0, label='预测值')
    plt.plot(t2[:-2 * I], Data002[:-2*I], color='k', linewidth=1.0, label='实际值')
    plt.legend(loc=1)
    plt.xlabel("时间/s")
    plt.ylabel("倾角/°")
    plt.subplot(3, 1, 3)
