...
f1 = 'shuju.csv'  # A数据
f3 = 'shuju0.csv'     # 测试集数据
F = r"C:\Users\LDK\Desktop\da-tu\AT-LSTM\3"  # 图片保存位置
f_1 = r"C:\Users\LDK\Desktop\da-tu\AT-LSTM\3\rmse.txt"  # print输出保存位置
N1 = 50  # 两变量LSTM训练次数
N2 = 2  # 三变量LSTM训练次数
M = 5  # 训练步长
I = 4  # 输入
...
def train_test(f):
    ...
    data_csv_1 = pd.read_csv(f, usecols=[1])

    Data0 = data_csv_0.values.flatten()
    t = data_csv_1.values.flatten()
    # t = []
    # for i000 in range(len(Data0)):
    #     t.append((i000+1)*0.5)
    ...
    plt.plot(t[I:], Data0[I:], color='k', linewidth=1.0)
    plt.xlabel("时间/s")
    plt.ylabel("倾角/°")
    plt.subplot(3, 1, 3)
    plt.title("预测误差")
...
def text(f3, model0):
    ...
    data_csv_021 = pd.read_csv(f3, usecols=[1]).dropna()

    Data02 = data_csv_02.values.flatten()
    t = data_csv_021.values.flatten()
    # t = []
    # for i0001 in range(len(Data02)):
    #     t.append((i0001 + 1) * 0.5)
    ...
    plt.plot(t[I:], data020[I:], color='k', linewidth=1.0)
    plt.xlabel("时间/s")
    plt.ylabel("倾角/°")
    plt.subplot(3, 1, 3)
    plt.title("测试集预测误差")
