...
def attention_3d_block(inputs, train_X_e):
    # inputs.shape = (batch_size, time_steps, input_dim)
    input_dim = int(inputs.shape[2])
    a = Permute((2, 1))(train_X_e)
...
def model_attention_applied_before_lstm(train_X_e):
    K.clear_session() #清除之前的模型，省得压满内存
    inputs = Input(shape=(I, INPUT_DIM,))
    attention_mul = attention_3d_block(inputs, train_X_e)
    lstm_units = 32
...
def trainModel_at(train_X, train_Y, N, train_X_e):
    '''
    trainX，trainY: 训练LSTM模型所需要的数据
    '''
    model = Sequential()
    model.add(model_attention_applied_before_lstm(train_X_e))
    # model.add(LSTM(
    #     140,  # 输出维度？
    #     input_shape=(train_X.shape[1], train_X.shape[2]),
    #     return_sequences=True))  # return_sequences神将网络的层数，不是最后一层=true，最后一层=false
...
def train_test(f):
    ...
    plt.subplot(3, 1, 1)
    plt.title("训练集预测")
    plt.plot(y01_2[:-O], color='gray', linewidth=1.0)
    plt.plot(Data0[I+O:], color='k', linewidth=1.0)
    plt.xlabel("时间/s")
    plt.ylabel("波高/m")
    plt.subplot(3, 1, 3)
    plt.title("训练集预测误差")
...
def text(f3, model0):
    ...
    plt.subplot(3, 1, 1)
    plt.title("测试集预测")
    plt.plot(y02_2[:-O], color='gray', linewidth=1.0)
    plt.plot(data020[I+O:], color='k', linewidth=1.0)
    plt.xlabel("时间/s")
    plt.ylabel("波高/m")
    plt.subplot(3, 1, 3)
    plt.title("测试集预测误差")
...
def train_text2(D_d01,D01):
    ...
    delta1 = np.zeros(len(delta_t1) * 1)
    delta1.dtype = 'float64'
    delta1 = delta1.reshape(len(delta_t1), 1)
    # data1[:, 0] = T1  # 时间
    delta1[:, 0] = delta_t1  # 位移

    # 画图
    plt.subplot(3, 1, 1)
    plt.title("本地训练数据")
    plt.plot(data1, color='k', linewidth=1.0)
    plt.xlabel("时间/s")
    plt.ylabel("波高/m")
    plt.subplot(3, 1, 3)
    plt.title("训练数据误差")
    plt.plot(delta1, color='k', linewidth=1.0)
    plt.xlabel("个数")
    plt.ylabel("时间/s")
    ...
    delta1, normalize100 = NormalizeMult(delta1)
    train_X1, train_Y1 = create_dataset(data1, I, O)
    train_X1_e, train_Y1_e = create_dataset(delta1, I, O)
    model1 = trainModel_at(train_X1, train_Y1, N2, train_X1_e)
    ...
    for i in range((len(Data1) - I)):
        ...
        delta11 = np.zeros(I * 1)
        delta11.dtype = 'float64'
        delta11 = data11.reshape(I, 1)
        data11[:, 0] = Data1[i:i + I]
        delta11[:, 0] = delta_t1[i:i + I]

        # 测试
        data11, normalize11 = NormalizeMult(data11)
        delta11, normalize1100 = NormalizeMult(delta11)
        test_X11 = data11.reshape(1, I, 1)
        test_X11_e = delta11.reshape(1, I, 1)
        y_hat11 = model1.predict(test_X11, test_X11_e)
