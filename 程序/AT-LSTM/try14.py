...
f_1001 = r"C:\Users\刘敦康\Desktop\datu\new\数据.txt"
N1 = 20  # 两变量LSTM训练次数
N2 = 2  # 三变量LSTM训练次数
M = 5  # 训练步长
I = 4  # 输入
...
def attention_3d_block0(inputs):
    ...
    output_attention_mul = Multiply()([c, b_probs])
...
def create_dataset(data, n_predictions, n_next):  # n_predictions：输入数据个数    n_next：输出数据个数
    ...
    for i in range(data.shape[0] - n_predictions - n_next - 1):
        ...
        b = []
        for i0 in range(n_predictions):
            b.append(i0)
        c = np.array(b).reshape(n_predictions, 1)
        d = np.hstack([a, c])
        # print(a)
        # print(d)
        train_X.append(d)
...
def train_test(f):
    # 读取数据(训练)
    data_csv_0 = pd.read_csv(f, usecols=[2])  # 读取第一列数据  位移
    data_csv_1 = pd.read_csv(f, usecols=[0])

    Data0 = data_csv_0.values.flatten()[:5000]
    t = data_csv_1.values.flatten()[:5000]
    ...
    plt.subplot(3, 1, 1)
    plt.title("训练数据")
    plt.plot(t, data0[:, 0], color='k', linewidth=1.0)
    plt.xlabel("时间/s")
    plt.ylabel("倾角/°")
    plt.subplot(3, 1, 3)
    plt.title("训练数据时间差")
    ...
    model0 = trainModel0(train_X0, train_Y0, N1)
    ...
    for i in range((len(Data0) - I)):
        ...
        for i0 in range(I):
            a0.append(i0)
        b0 = np.array(a0).reshape(I, 1)
        c = np.hstack([data01, b0])
        test_X01 = c.reshape(1, I, 2)
    ...
    plt.subplot(3, 1, 1)
    plt.title("结果预测")
    plt.plot(t[I:], y01_2, color='gray', linewidth=1.0, label='预测值')
    plt.plot(t[I:], Data0[:-I], color='k', linewidth=1.0, label='实际值')
    plt.legend(loc=1)
    plt.xlabel("时间/s")
    plt.ylabel("倾角/°")
    plt.subplot(3, 1, 3)
...
def text(f3, model0):
    # 读取数据(预测)
    data_csv_02 = pd.read_csv(f3, usecols=[2]).dropna()  # 读取第一列数据
    data_csv_021 = pd.read_csv(f3, usecols=[0]).dropna()

    Data02 = data_csv_02.values.flatten()[-2500:]
    t = data_csv_021.values.flatten()[-2500:]
    ...
    for i in range((len(data020) - I)):
        ...
        for i0 in range(I):
            a0.append(i0)
        b0 = np.array(a0).reshape(I, 1)
        c = np.hstack([data02, b0])
        test_X02 = c.reshape(1, I, 2)
    ...
    # print("数据：" + str(data020[:-I]), file=f0110)
    # print("预测：" + str(y02_2), file=f0110)
    # print("误差：" + str(D02), file=f0110)
    # print("时间：" + str(t[I:]), file=f0110)
    DDD0 = pd.DataFrame(data020[:-I])
    DDD0.to_excel(writer0, 'page_1', float_format='%.5f')
    writer0.save()
    DDD1 = pd.DataFrame(y02_2)
    DDD1.to_excel(writer1, 'page_1', float_format='%.5f')
    writer1.save()
    DDD2 = pd.DataFrame(D02)
    DDD2.to_excel(writer2, 'page_1', float_format='%.5f')
    writer2.save()
    DDD3 = pd.DataFrame(t[I:])
    ...
    plt.subplot(3, 1, 1)
    plt.title("测试集预测")
    plt.plot(t[I:], y02_2, color='gray', linewidth=1.0, label='预测值')
    plt.plot(t[I:], data020[:-I], color='k', linewidth=1.0, label='实际值')
    plt.legend(loc=1)
    plt.xlabel("时间/s")
    plt.ylabel("倾角/°")
    plt.subplot(3, 1, 3)
