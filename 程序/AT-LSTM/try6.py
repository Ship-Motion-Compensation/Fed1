...
def model_attention_applied_before_lstm0():
    ...
    attention_mul = LSTM(lstm_units, return_sequences=True)(attention_mul)
    attention_mul = LSTM(lstm_units, return_sequences=False)(attention_mul)
    output = Dense(20, activation='sigmoid')(attention_mul)
    model = Model(input=[inputs], output=output)
    return model
...
def trainModel_at0(train_X, train_Y, N):
    '''
    trainX，trainY: 训练LSTM模型所需要的数据
    '''
    model = Sequential()
...
def train_test(f):
    ...
    model0 = trainModel_at(train_X0, train_Y0, N1)
...
def train_text2(D_d01,D01):
    ...
    model1 = trainModel_at0(train_X1, train_Y1, N2)
