...
def model_attention_applied_before_lstm0():
    K.clear_session() #清除之前的模型，省得压满内存
    inputs = Input(shape=(I, INPUT_DIM,))
    attention_mul, C = attention_3d_block0(inputs)
    lstm_units = 32
    ...
    model = Model(input=[inputs], output=output)
    model.summary()
    attention_vector = get_activations(model, C,
...
def train_test(f):
    ...
    params = model0.get_weights()  # 读取权值
    # 创建测试数组
    y01_2 = []
    ...
    plt.show()
    return Data0[I:], D01, model0, params
...
if __name__=="__main__":
    ...
    D_d01, D01, model0, params = train_test(f1)
    print(params)
