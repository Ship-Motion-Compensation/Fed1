...
def attention_3d_block0(inputs):
    # inputs.shape = (batch_size, time_steps, input_dim)
    input_dim = int(inputs[:, 0].shape[2])
    a = Permute((2, 1))(inputs[:, 0])
    a = Reshape((input_dim, I))(a) # this line is not useful. It's just to know which dimension is what.
    a = Dense(I, activation='softmax')(a)
    b = softmax(inputs[:, 0], axis=1)
    if SINGLE_ATTENTION_VECTOR:
        a = Lambda(lambda x: K.mean(x, axis=1), name='dim_reduction')(a)
        a = RepeatVector(input_dim)(a)
    a_probs = Permute((2, 1), name='attention_vec')(a)
    output_attention_mul = Multiply()([inputs[:, 0], a_probs])
    output_attention_mul0 = Multiply()(output_attention_mul, b)
...
def trainModel0(train_X, train_Y, N):
    ...
    reduce_lr = ReduceLROnPlateau(monitor='loss', patience=5, mode='auto', factor=0.5)
    history = model.fit(train_X[:, 0], train_Y[:, 0], epochs=N, batch_size=M, verbose=2, shuffle=False, callbacks=[reduce_lr])  # verbose = 1 为输出进度条记录,日志显示；verbose = 2 为每个epoch输出一行记录
