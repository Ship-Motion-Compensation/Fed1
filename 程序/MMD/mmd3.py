...
# f3 = r'C:\Users\LDK\Desktop\数据\6\4-15-6.csv'     # 测试集数据
...
for i0 in range(len(path_list)):
    f3 = os.path.join(path, path_list[i0])
    # print(path_list[i0])

    data_csv_0 = pd.read_csv(f1, usecols=[4])  # 读取第一列数据  位移
    Data00 = data_csv_0.values.flatten()[200:1200]

    data_csv_01 = pd.read_csv(f3, usecols=[4])  # 读取第一列数据  位移
    Data001 = data_csv_01.values.flatten()[200:1200]
    ...
    for i in range(int(O)):
        D00 = data_csv_0.values.flatten()[500 + (I * i):500 + (I * (i + 1))]
        D0.append(D00)
        D01 = data_csv_01.values.flatten()[500 + (I * i):500 + (I * (i + 1))]
        D1.append(D01)
